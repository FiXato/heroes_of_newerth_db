...
To get a list of all items and save them as an HTML file, run the following from the command line:

 ./get_items > items.html

To get the first item, run the following from the command line:

 ./get_items first

And to get a specific item, find its detail page url and feed it as such:
...
* Find out and fix why the Storm Spirit isn't parsing the required-items.
